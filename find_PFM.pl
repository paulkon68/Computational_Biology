# This script computes the PFM and the consensus sequence of a file with same length sequences (ten_dna_sequences.txt). After that, it saves both data in a file (PFM.txt).
